...
export function showRepoStatus(status: string, type: string) {
  if (type == "proxy") {
    if (status == 0) {
      return "Offline-Repository Offline";
    } else {
      return "Online-Ready to Connect";
    }
  } else {
    if (status == 0) {
      return "Offline";
...
export function showBlobStoresState(state: string) {
  if (state == 0) {
    return "End";
  } else {
    return "Started";
