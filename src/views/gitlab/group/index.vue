<script setup lang="ts">
  ...
  const getList = ()=>{
    ...
    loading.value = true;
    pageList(addDateRangeRuoyi(queryParams, dateRange.value))
    .then(response => {
          loading.value = false
          if(response?.data?.records.length > 0){
            groupList.splice(0,groupList.length);
            Object.assign(groupList, response?.data?.records)
  ...
  const handleStatusChange = (row)=>{
      ...
      ElMessageBox.confirm(
        '确认要"' + text + '""' + row.gitlabGroupName + '"组吗？',
        'Warning',
        {
      ...
      changeGroupStatus(row.id, row.status);
        ElMessage({
          type: 'success',
          message: text + '成功',
        })
  ...
  <div class="main-wrapp">
    ...
    <div class="option-wrap">
      ...
      <el-button
        ...
        size="default"
        :disabled="single"
        @click="handleUpdate" v-hasPerms="['/gitlab/group/add']" ><el-icon><EditPen /></el-icon>修改</el-button>  
