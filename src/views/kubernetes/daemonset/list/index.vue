<script setup lang="ts">
  ...
  import { pageList,nameSpaceList,removeDaemonSet,changeStatus} from "@/api/kubernetes/daemonset"
  ...
  const handleDetail = function(row){
    router.push({path : "/kubernetes/daemonset/operate", query:{ instanceCode: defaultInstanceCode.value ,id: row.id} })
  }
  ...
  const handleDelete = function(row){
    ...
    ElMessageBox.confirm(
      ...
      removeDaemonSet(row.id).then((res)=>{
          if(res.code == 200){
              // 重置查询表单,并进行查询
  ...
  const btnList = ref([
    {
      btnName: '查看',
      permArray: ['/kubernetes/daemonset/detail/*'],
      isShow: () => true,
      isDisable: false,
    ...
    },
    {
      ...
      permArray: ['/kubernetes/daemonset/changeStatus/**'],
      isShow: () => true,
      isDisable: false,
    ...
    {
      btnName: '删除',
      permArray: ['/kubernetes/daemonset/del/*'],
      isShow: () => true,
      isDisable: false,
    ...
    <yt-card>
        <div class="option-wrap">
          <el-button
            ...
            plain
            size="default"
            @click="handleAdd" v-hasPerms="['/kubernetes/daemonset/add']" ><el-icon><Plus /></el-icon>新增</el-button>
        </div>
      <!--table  -->
            ...
            <!-- <template #default="scope">
              <div class="action-btn">
                <el-button
                  ...
                  v-hasPerms="['/kubernetes/daemonset/detail/*']"
                >查看</el-button>
                <el-button
                  size="small"
                  :icon="getStatusIcon(scope.row)"
                  @click="handleStatusChange(scope.row)"
                  v-hasPerms="['/kubernetes/daemonset/changeStatus/**']"
                >{{ showStatusOperateFun(scope.row.status)  }}</el-button>
                <el-button
                  size="small"
                  icon="Delete"
                  @click="handleDelete(scope.row)"
                  v-hasPerms="['/kubernetes/daemonset/del/**']"
                >删除</el-button>
              </div>
            </template> -->
